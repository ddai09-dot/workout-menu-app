...
test "$(shasum -a 256 implementation-v0.9.1.zip | awk '{print $1}')" = "$expected_v091"
unzip -t implementation-v0.9.1.zip >/dev/null
rm -rf app && mkdir app && unzip -q implementation-v0.9.1.zip -d app
build_and_extract() {
  local builder="$1" output="$2" verifier="$3"
  python3 "$builder" app "$output"
  rm -rf app && mkdir app && unzip -q "$output" -d app
...
build_and_extract tools/task20_d2i_build_canonical_v098.py implementation-v0.9.8.zip tools/task20_d2i_verify_canonical_v098.py
build_and_extract tools/task20_d2i_build_canonical_v099.py implementation-v0.9.9.zip tools/task20_d2i_verify_canonical_v099.py
