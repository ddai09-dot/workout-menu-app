...
def main():
    if len(sys.argv)!=2: raise SystemExit('Usage: task20_d2j_verify_canonical_v0916.py <app-root>')
    ...
    req(readme,'実装基盤 v0.9.16','README version')
    req(readme,'v0.9.15','immutable predecessor history')
    req(readme,'D2-05〜D2-07の未網羅分','partial-scope remainder wording')
    matrix=(root/'docs/VERSION_MATRIX.md').read_text(encoding='utf-8')
    req(matrix,'現在のプロジェクト版：`0.9.16+34`','matrix version')
    req(matrix,'| 0.9.16 |','matrix v0.9.16 history')
    for path in ('README.md','docs/VERSION_MATRIX.md','docs/IMPLEMENTATION_ROADMAP.md'):
        txt=(root/path).read_text(encoding='utf-8')
        if 'D2-09残件' in txt: raise SystemExit(f'Stale D2-09 pending wording remains in {path}')
    ...
    result={'status':'PASS','task':'Task20-D2J harness follow-up + canonical status consistency','canonical_package':'implementation-v0.9.16.zip','app_version':'0.9.16+34','parent_canonical_version':'0.9.15+33','expected_flutter_test_count':56,'schema_version':9,'schema_table_count':75,'product_lib_changed_from_v0_9_15':False,'tests_changed_from_v0_9_15':False,'schema_changed_from_v0_9_15':False,'assets_changed_from_v0_9_15':False,'canonical_file_count':len(actual_files),'weekly_planner_sha256':W,'workout_adjustment_sha256':ADJ,'tree_hashes':actual,'tree_file_counts':{'runtime':nr,'product_lib':nl,'tests':nt,'schema':ns,'assets':na},'d2j_acceptance_status':'PENDING_EXACT_CURRENT_HEAD_CI_AND_ARTIFACT_AUDIT','task20_d2_fully_verified':False,'physical_device_verified':False,'native_accessibility_verified':False}
