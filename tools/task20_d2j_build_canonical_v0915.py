...
EXPECTED='ccf69d61ee4f98e7c3a0e8926ab42c53cf868f133303d573b1d30896d6119b60'
PATCH_SHA='e2097e1b5c0c6489628357b1661b2800a8044bed9be8ae7bace9777643ee9a0b'
...
def apply(root: Path) -> None:
    patch_b64=Path(__file__).with_name('task20_d2j_v0915.patch.gz.b64')
    ...
    try:
        ...
        raise SystemExit(f'Unable to decode v0.9.15 patch: {exc}')
    actual=hashlib.sha256(patch).hexdigest()
    if actual!=PATCH_SHA: raise SystemExit(f'v0.9.15 patch SHA mismatch: {actual} != {PATCH_SHA}')
    with tempfile.NamedTemporaryFile(prefix='task20-d2j-v0915-',suffix='.patch',delete=False) as handle:
    ...
    try:
        ...
        if check.returncode!=0: raise SystemExit(f'v0.9.15 patch precondition failed:\n{check.stdout}{check.stderr}')
        run=subprocess.run(['git','apply',str(patch_path)],cwd=root,text=True,capture_output=True)
        if run.returncode!=0: raise SystemExit(f'v0.9.15 patch apply failed:\n{run.stdout}{run.stderr}')
    finally:
        patch_path.unlink(missing_ok=True)

def main() -> int:
    if len(sys.argv)!=3: raise SystemExit('Usage: task20_d2j_build_canonical_v0915.py <v0.9.13-app-root> <output-zip>')
    src=Path(sys.argv[1]).resolve(); out=Path(sys.argv[2]).resolve()
    if 'version: 0.9.13+31' not in (src/'pubspec.yaml').read_text(encoding='utf-8'):
        raise SystemExit('requires canonical v0.9.13 / 0.9.13+31')
    with tempfile.TemporaryDirectory(prefix='task20-d2j-v0915-') as td:
        root=Path(td)/'app'; shutil.copytree(src,root); apply(root)
        if 'version: 0.9.15+33' not in (root/'pubspec.yaml').read_text(encoding='utf-8'):
            raise SystemExit('v0.9.15 patch did not set app version')
