...
LOG_DIR="${TASK20_D2F_LOG_DIR:-$APP_DIR/build/task20_d2f_records_progression}"
RESULT_LINES="$LOG_DIR/device_results.ndjson"
DRIVE_TIMEOUT_SECONDS="${TASK20_D2F_DRIVE_TIMEOUT_SECONDS:-1500}"
MAX_STARTUP_ATTEMPTS="${TASK20_D2F_MAX_STARTUP_ATTEMPTS:-2}"
APP_BUNDLE="$APP_DIR/build/ios/iphonesimulator/Runner.app"
mkdir -p "$LOG_DIR"
: > "$RESULT_LINES"

if [[ "$(uname -s)" != "Darwin" ]]; then
  echo "ERROR: Task 20-D2F iOS UI acceptance requires macOS." >&2
  exit 2
fi
...
python3 "$ROOT/tools/task20_d2f_prepare_ui_acceptance.py" "$APP_DIR"
(
  set -x
  cd "$APP_DIR"
  flutter pub get
    ...
    TASK20_D2_SCREENSHOT_DIR="$screenshot_dir" \
      python3 "$ROOT/tools/task20_d2a_run_with_timeout.py" \
        ...
        flutter drive \
          ...
          --target=integration_test/task20_d2f_records_progression_test.dart \
          -d "$udid"
  )
}
...
is_retryable_startup_failure() {
  local log_file="$1"
  local screenshot_dir="$2"
  [[ -z "$(find "$screenshot_dir" -type f -name 'D2F_*.png' -print -quit)" ]] && \
    grep -Eqi \
...
if len(devices) != 2 or any(item.get("status") != "PASS" for item in devices):
    raise SystemExit("Expected two passing Task 20-D2F device results")
if any(len(item.get("screenshots", [])) != 8 for item in devices):
    raise SystemExit("Expected eight Task 20-D2F screenshots per device")
result = {
    "task": "Task 20-D2F records and progression acceptance",
    "status": "PASS",
    ...
    "devices": devices,
    "verified_cases": ["D2-06-partial", "D2-10-partial"],
    "verified_behaviors": [
        "partial workout is reflected in records dashboard and recent sessions",
        "workout history and session detail preserve exercise and set values",
        "exercise summary and same-series detail are readable",
        "manual body measurement persists and refreshes the dashboard",
        "no-proposal state is displayed without an error",
        "repository values match the visible records flow",
    ],
    "excluded_from_full_pass": [
        "immutable-history evidence after later menu regeneration",
        "body-measurement correction and supersession audit",
        "progression proposal accept, keep, defer, and reject decisions",
        "multiple equipment, variation, and side series separation",
        "process termination after records input",
    ],
    "startup_retry_policy": {
        "maximum_attempts": int(os.environ["MAX_STARTUP_ATTEMPTS"]),
        "retry_scope": "pre-test debug launch failures with no D2F screenshot only",
...
echo "Task 20-D2F records and progression acceptance passed."
