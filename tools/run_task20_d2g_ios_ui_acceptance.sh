...
LOG_DIR="${TASK20_D2G_LOG_DIR:-$APP_DIR/build/task20_d2g_my_page_settings}"
RESULT_LINES="$LOG_DIR/device_results.ndjson"
DRIVE_TIMEOUT_SECONDS="${TASK20_D2G_DRIVE_TIMEOUT_SECONDS:-1500}"
MAX_STARTUP_ATTEMPTS="${TASK20_D2G_MAX_STARTUP_ATTEMPTS:-2}"
APP_BUNDLE="$APP_DIR/build/ios/iphonesimulator/Runner.app"
mkdir -p "$LOG_DIR"
: > "$RESULT_LINES"

if [[ "$(uname -s)" != "Darwin" ]]; then
  echo "ERROR: Task 20-D2G iOS UI acceptance requires macOS." >&2
  exit 2
fi
    ...
    TASK20_D2_SCREENSHOT_DIR="$screenshot_dir" \
      python3 "$ROOT/tools/task20_d2a_run_with_timeout.py" \
        ...
        flutter drive \
          --keep-app-running \
          --no-dds \
          --driver=test_driver/task20_d2g_driver.dart \
          --target=integration_test/task20_d2g_my_page_settings_test.dart \
          -d "$udid"
  )
}
...
is_retryable_startup_failure() {
  local log_file="$1"
  local screenshot_dir="$2"
  [[ -z "$(find "$screenshot_dir" -type f -name 'D2G_*.png' -print -quit)" ]] && \
    grep -Eqi \
...
PY
  ...
  for screenshot_name in \
    ...
    D2G_07_faq.png \
    D2G_08_data_reset_entry.png; do
...
if len(devices) != 2 or any(item.get("status") != "PASS" for item in devices):
    raise SystemExit("Expected two passing Task 20-D2G device results")
if any(len(item.get("screenshots", [])) != 8 for item in devices):
    raise SystemExit("Expected eight Task 20-D2G screenshots per device")
result = {
    "task": "Task 20-D2G My Page settings acceptance",
    "status": "PASS",
    ...
    "devices": devices,
    "verified_cases": ["D2-07-partial", "D2-10-partial"],
    "verified_behaviors": [
        "all seven training-setting sections open with unchanged save disabled",
        "unsaved split changes require explicit discard confirmation",
        "goal changes persist for the next menu without mutating the finalized current menu",
        "restriction increases prompt for current-week review without automatic menu mutation",
        "NEXT_MENU application boundary is preserved",
        "FAQ and local-data entry routes are readable",
    ],
    "excluded_from_full_pass": [
        "saving every field combination in all seven sections",
        "environment equipment reduction and capacity reduction branches",
        "weekly planner review acceptance after choosing the review action",
        "save failure injection",
        "settings process termination and restart",
        "physical device, Dynamic Type, and native accessibility",
    ],
    "startup_retry_policy": {
        "maximum_attempts": int(os.environ["MAX_STARTUP_ATTEMPTS"]),
        "retry_scope": "pre-test debug launch failures with no D2G screenshot only",
...
echo "Task 20-D2G My Page settings acceptance passed."
