...
def main():
    if len(sys.argv)!=2: raise SystemExit('Usage: task20_d2j_verify_canonical_v0913.py <app-root>')
    ...
    req((root/'README.md').read_text(encoding='utf-8'),'実装基盤 v0.9.13','README version')
    req((root/'docs/VERSION_MATRIX.md').read_text(encoding='utf-8'),'現在のプロジェクト版：`0.9.13+31`','matrix version')
    ...
    result={'status':'PASS','task':'Task20-D2J workout-adjustment enlarged-text width fix','canonical_package':'implementation-v0.9.13.zip','app_version':'0.9.13+31','parent_version':'0.9.12+30','expected_flutter_test_count':56,'schema_version':9,'schema_table_count':75,'schema_changed_from_v0_9_12':False,'assets_changed_from_v0_9_12':False,'canonical_file_count':len(actual_files),'weekly_planner_sha256':W,'workout_adjustment_sha256':ADJ,'tree_hashes':actual,'tree_file_counts':{'runtime':nr,'product_lib':nl,'tests':nt,'schema':ns,'assets':na},'d2j_acceptance_status':'PENDING_CURRENT_HEAD_CI_AND_ARTIFACT_AUDIT','task20_d2_fully_verified':False,'physical_device_verified':False,'native_accessibility_verified':False}
