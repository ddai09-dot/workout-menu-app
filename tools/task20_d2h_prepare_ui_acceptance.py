...
def main() -> int:
    if len(sys.argv) != 2:
        raise SystemExit("usage: task20_d2h_prepare_ui_acceptance.py <app-dir>")
    ...
    source_files = {
        ...
        repo_root / "tools" / "task20_d2h_restart_persistence_prepare_test.dart":
            app_dir / "integration_test" / "task20_d2h_restart_persistence_prepare_test.dart",
        repo_root / "tools" / "task20_d2h_restart_persistence_verify_test.dart":
            app_dir / "integration_test" / "task20_d2h_restart_persistence_verify_test.dart",
        repo_root / "tools" / "task20_d2h_driver.dart":
            app_dir / "test_driver" / "task20_d2h_driver.dart",
    }
    ...
    print(f"Prepared Task 20-D2H test-only overlay in {app_dir}")
    return 0
