...
LOG_DIR="${TASK20_D2C_LOG_DIR:-$APP_DIR/build/task20_d2c_onboarding_resume}"
RESULT_LINES="$LOG_DIR/device_results.ndjson"
DRIVE_TIMEOUT_SECONDS="${TASK20_D2C_DRIVE_TIMEOUT_SECONDS:-480}"
MAX_STARTUP_ATTEMPTS="${TASK20_D2C_MAX_STARTUP_ATTEMPTS:-2}"
APP_BUNDLE="$APP_DIR/build/ios/iphonesimulator/Runner.app"
mkdir -p "$LOG_DIR"
: > "$RESULT_LINES"

if [[ "$(uname -s)" != "Darwin" ]]; then
  echo "ERROR: Task 20-D2C iOS UI acceptance requires macOS." >&2
  exit 2
fi
...
python3 "$ROOT/tools/task20_d2c_prepare_ui_acceptance.py" "$APP_DIR"
(
  set -x
  cd "$APP_DIR"
  flutter pub get
  ...
  (
    ...
    TASK20_D2_SCREENSHOT_DIR="$screenshot_dir" \
      python3 "$ROOT/tools/task20_d2a_run_with_timeout.py" \
        ...
        flutter drive \
          --no-dds \
          --driver=test_driver/task20_d2c_driver.dart \
          --target="$target" \
          -d "$udid"
  )
}
...
while IFS=$'\t' read -r role udid runtime device_name; do
  ...
  echo "Task 20-D2C starting: role=$role device=$device_name runtime=$runtime udid=$udid"
  ...
  for attempt in $(seq 1 "$MAX_STARTUP_ATTEMPTS"); do
    ...
    run_drive_phase \
      ...
      integration_test/task20_d2c_onboarding_resume_prepare_test.dart \
      "$phase1_log" \
      "$phase1_result_file"
    phase1_code="$?"
    set -e
    ...
    if [[ "$phase1_code" -ne 0 ]]; then
      ...
      if is_retryable_startup_failure \
        "$phase1_log" 'D2C_01_saved_midway.png' "$attempt_screenshot_dir"; then
        retryable_startup_failure=true
      fi
      if [[ "$retryable_startup_failure" == true && "$attempt" -lt "$MAX_STARTUP_ATTEMPTS" ]]; then
        echo "Task 20-D2C phase 1 startup infrastructure failure; retrying clean attempt."
        continue
      fi
...

    {
      ...
      echo "terminate_after_phase1=true"
      if xcrun simctl terminate "$udid" "$BUNDLE_ID"; then
        echo "initial_terminate=PASS"
      else
        echo "initial_terminate=APP_NOT_RUNNING"
        xcrun simctl launch "$udid" "$BUNDLE_ID"
        sleep 2
        xcrun simctl terminate "$udid" "$BUNDLE_ID"
        echo "launch_then_terminate=PASS"
      fi
    } > "$process_log" 2>&1
    ...
    run_drive_phase \
      "$udid" \
      "$attempt_screenshot_dir" \
      integration_test/task20_d2c_onboarding_resume_verify_test.dart \
      "$phase2_log" \
      "$phase2_result_file"
    phase2_code="$?"
    set -e
    ...
    if [[ "$phase2_code" -ne 0 ]]; then
      ...
      retryable_startup_failure=false
      if is_retryable_startup_failure \
        "$phase2_log" 'D2C_0[2-5]_*.png' "$attempt_screenshot_dir"; then
        retryable_startup_failure=true
      fi
      if [[ "$retryable_startup_failure" == true && "$attempt" -lt "$MAX_STARTUP_ATTEMPTS" ]]; then
        echo "Task 20-D2C phase 2 startup infrastructure failure; retrying clean two-phase attempt."
        continue
      fi
  ...
  if [[ "$final_code" -ne 0 ]]; then
    ROLE="$role" DEVICE_NAME="$device_name" RUNTIME="$runtime" UDID="$udid" \
      ...
      ATTEMPTS="$attempted_count" RETRYABLE_STARTUP_FAILURE="$retryable_startup_failure" \
      python3 - <<'PY'
import json
import os
...
if len(devices) != 2 or any(item.get("status") != "PASS" for item in devices):
    raise SystemExit("Expected two passing Task 20-D2C device results")
result = {
    "task": "Task 20-D2C onboarding resume acceptance",
    "status": "PASS",
    ...
    "devices": devices,
    "verified_cases": ["D2-02", "D2-10-partial"],
    "verified_behaviors": [
        "draft saved beyond three onboarding screens",
        "OS-level process termination between phases",
        "resume prompt after relaunch",
        "current step restored",
        "nickname and age restored",
        "reset cancellation preserves values",
        "reset confirmation returns to intro",
    ],
...
echo "Task 20-D2C onboarding resume acceptance passed."
